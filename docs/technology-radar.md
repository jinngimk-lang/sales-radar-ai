...
| `browser-use/browser-use` | `0.13.8`, released 2026-08-16; active development and recent runtime/test issues are visible upstream | MIT | DEFERRED | Browserbase is the healthy production browser path. Keep Browser Use as an alternate-runtime candidate only for a concrete Browserbase limitation or validated local/browser-agent workflow. |
| `@livekit/agents` | `1.7.0`, published 2026-08-20; active package with OpenTelemetry dependencies | Apache-2.0 | WATCH | Strong candidate for future realtime communication. Before adoption, require explicit transcript/recording retention, observability and PII-redaction controls; do not treat provider redaction as the only privacy boundary. |
