...
Sales Radar therefore does not adopt MCP v2 or a newer Exa MCP package by default. Any future MCP-backed provider or orchestration runtime must pin a supported major/version, verify initialize/tool/resource behavior and auth/telemetry semantics in isolation, exercise cancellation/reconnect recovery where applicable, and preserve an adapter-level rollback path. The current Crawl4AI enrichment path remains unchanged because there is no evidence that production depends on its affected Docker/MCP bridge.

## Scan record — 2026-08-26

- Main head `62397850bc7e750e538f0370ace2964b57003364` is deployment-healthy: Vercel and both Railway commit statuses are successful; Production Action Smoke run #136 also completed successfully. No production regression was observed.
- Draft PR #39 remains isolated feature work. It was not merged or altered during this radar update.
...
- Re-verified the Browser Use action-result evidence: #5486 is the failed tab-switch-reported-success bug, while #5469 remains the failed click/dropdown-reported-success case. Status remains `DEFERRED` while Browserbase is healthy.
